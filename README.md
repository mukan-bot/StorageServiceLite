...
ブラウザーだけで `.ssobj` ファイルを閲覧できる静的Web UIを `webui/` に用意しています。

1. `webui/index.html` をブラウザーで開きます。
...
GitHub Pagesで公開する場合は、リポジトリの Settings → Pages → Build and deployment の Source を `GitHub Actions` に設定してください。`main` または `master` ブランチへ `webui/` や `.github/workflows/deploy-webui.yml` の変更が push されると、`webui/` の内容が静的ページとしてデプロイされます。
